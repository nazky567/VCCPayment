        ...
        try {
            ...
            if ($status_response) {
                ...
                if ($transaction_status === 'settlement' || $transaction_status === 'capture') {
                    ...
                    $stmtAudit->execute(['System_Redirect_Fallback', 'Update Transaction Status', "Transaction $order_id updated to $db_status via success redirect check.", $_SERVER['REMOTE_ADDR']]);
...
// Route based on actual transaction status to prevent displaying success UI for pending/failed transactions
...
if ($current_status === 'pending') {
    ...
    exit;
} elseif ($current_status !== 'settlement' && $current_status !== 'capture' && $current_status !== 'success') {
    header("Location: failed.php?order_id=" . urlencode($order_id));
    exit;
}
?>
...
<head>
    ...
    <title>Pembayaran Sukses - CloudPay Sandbox</title>
    <!-- Bootstrap 5 CSS -->
            ...
            <div class="col-md-6">
                <div class="card card-custom text-center">
                    <div class="card-body p-5">
                        <div class="mb-4">
                            <span class="d-inline-flex align-items-center justify-content-center bg-success text-white rounded-circle" style="width: 80px; height: 80px; font-size: 2.5rem; box-shadow: 0 0 20px rgba(16, 185, 129, 0.4);">
                                <i class="bi bi-check-lg"></i>
                            </span>
                        </div>
                        <h2 class="text-success fw-bold mb-2">Pembayaran Berhasil!</h2>
                        <p class="text-secondary small mb-4">Terima kasih, pembayaran Anda telah berhasil diproses oleh Midtrans Sandbox simulator.</p>
                        ...
                        <div class="p-3 rounded mb-4" style="background-color: rgba(255, 255, 255, 0.02); border: 1px solid var(--border-color);">
                            <div class="row g-2 text-start">
                                ...
                                <div class="col-6 text-secondary small">Metode Pembayaran</div>
                                <div class="col-6 text-end fw-semibold text-white"><?php echo htmlspecialchars($trx['payment_type'] ?: 'Sandbox simulator'); ?></div>

                                <div class="col-6 text-secondary small">Total Nominal</div>
                                <div class="col-6 text-end fw-bold text-success">Rp <?php echo number_format($trx['amount'], 0, ',', '.'); ?></div>
                            </div>
                        </div>

                        <div class="d-grid gap-2">
                            <!-- Link to export invoice PDF -->
                            <a href="admin/export.php?action=pdf&order_id=<?php echo urlencode($trx['order_id']); ?>" class="btn btn-gradient-primary py-3">
                                <i class="bi bi-file-earmark-pdf-fill me-2"></i> Download Invoice PDF
                            </a>
                            <a href="checkout.php" class="btn btn-outline-custom py-2">
